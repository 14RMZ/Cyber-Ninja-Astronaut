/* CYBER-NINJA ASTRONAUT - OBFUSCATED PRODUCTION BUILD (C) RMZ Games */
(function(_0x88a, _0x99b){
...
enemyBullets.push(new Bullet(bulletX, bulletY, direction, "#ff0000"));
...
if (player.invincibleTimer > 0 || isDevGodMode) return true;
if (player.lives > 1) {
player.lives--;
updateLivesHUD();
...
this.laserTimer++;
const lockFrame = this.phase === 3 ? 120 : 120;
...
this.cooloffTimer = this.phase === 3 ? 90 : 120;
}
...
const dy = player.y + player.height / 2 - cy;
const baseAngle = Math.atan2(dy, dx);
const speed = bossLevel >= 3 ? 5.5 : 4.5;
const spread = 0.25;
enemyShootSound.currentTime = 0;
enemyShootSound.play().catch(() => {});
...
bullets.push(new Bullet(player.x + player.width / 2, player.y + player.height / 2, player.direction, "#ffdd00"));
...
player.rocketMaxTimer = duration * 60;
...
player.rotation += player.direction * 0.18;
}
if (screenShakeTimer > 0) {
screenShakeTimer--;
}
